...
from keras_frcnn import config, rpn
...
parser.add_option("--source", dest="source", help="set the source of the training(rpn/all). def=rpn", default="rpn")
...
shared_layers = nn.base_vgg(img_input)
...
Callbacks=keras.callbacks.ModelCheckpoint("./models/rpn/rpn.weights.{epoch:02d}-{loss:.2f}.hdf5", monitor='loss', verbose=1, save_best_only=True, save_weights_only=True, mode='auto', period=2)

class MyCustomCallback(tf.keras.callbacks.Callback):

  def on_epoch_end(self, epoch, logs=None):
    #print('The average loss for epoch {} is {:7.2f}.'.format(epoch, logs['loss']))
    root[0][0].text = "./models/rpn/rpn.weights.{}-{:.2f}.hdf5".format(epoch, logs['loss'])
    root[0][1].text = "{:.4f}".format(logs['loss'])
    tree.write('weights.xml')

callback=[Callbacks, MyCustomCallback()]
...
numpy.savetxt("vgg16_rpn_loss_history.txt", numpy_loss_history, delimiter=",")
